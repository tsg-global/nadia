defmodule NadiaTest do
  ...
  setup_all do
    unless Application.get_env(:nadia, :token) do
      # this is purposely set to test the token config override via the options
      Application.put_env(:nadia, :token, {:system, "ENV_TOKEN", "ALT_TEST_TOKEN"})
    end
  ...
  test "get_me" do
    use_cassette "get_me" do
      {:ok, me} = Nadia.get_me %{}, token: @test_token
  ...
  test "send_message" do
    use_cassette "send_message" do
      {:ok, message} = Nadia.send_message(666, "aloha", %{}, token: @test_token)
  ...
  test "forward_message" do
    use_cassette "forward_message" do
      {:ok, message} = Nadia.forward_message(666, 666, 666, %{}, token: @test_token)
  ...
  test "send_photo" do
    use_cassette "send_photo" do
      ...
      {:ok, message} = Nadia.send_photo(666, file_id, %{}, token: @test_token)
  ...
  test "send_sticker" do
    use_cassette "send_sticker" do
      {:ok, message} =
        Nadia.send_sticker(666, "BQADBQADBgADmEjsA1aqdSxtzvvVAg", %{}, token: @test_token)
  ...
  test "send_contact" do
    use_cassette "send_contact" do
      {:ok, message} =
        Nadia.send_contact(666, 10123800555, "Test", %{}, token: @test_token)
  ...
  test "send_location" do
    use_cassette "send_location" do
      {:ok, message} =
        Nadia.send_location(666, 1, 2, %{}, token: @test_token)
  ...
  test "send_venue" do
    use_cassette "send_venue" do
      {:ok, message} =
        Nadia.send_venue(666, 1, 2, "Test", "teststreet", %{}, token: @test_token)
  ...
  test "delete_message" do
    use_cassette "delete_message" do
      assert Nadia.delete_message(1, 666, %{}, token: @test_token) == :ok
    end
  end

  test "send_chat_action" do
    use_cassette "send_chat_action" do
      assert Nadia.send_chat_action(666, "typing", %{}, token: @test_token) == :ok
    end
  end

  test "get_user_profile_photos" do
    use_cassette "get_user_profile_photos" do
      assert {:ok, user_profile_photos} =
        Nadia.get_user_profile_photos(666, %{}, token: @test_token)
  ...
  test "get_updates" do
    use_cassette "get_updates" do
      {:ok, updates} = Nadia.get_updates([limit: 1], token: @test_token)
  ...
  test "set webhook" do
    use_cassette "set_webhook" do
      assert Nadia.set_webhook([url: "https://telegram.org/"], token: @test_token) == :ok
    end
  end
  ...
  test "get webhook info" do
    use_cassette "get_webhook_info" do
      ...
      assert Nadia.get_webhook_info(%{}, token: @test_token) == {:ok, webhook_info}
    end
  end

  test "delete webhook" do
    use_cassette "delete_webhook" do
      assert Nadia.delete_webhook(%{}, token: @test_token) == :ok
    end
  end

  test "get_file" do
    use_cassette "get_file" do
      {:ok, file} = Nadia.get_file("BQADBQADBgADmEjsA1aqdSxtzvvVAg", %{}, token: @test_token)
  ...
  test "get_file_link" do
    ...
    {:ok, file_link} = Nadia.get_file_link(file, token: @test_token)
  ...
  test "get_chat" do
    use_cassette "get_chat" do
      assert {:ok, chat} = Nadia.get_chat("@group", %{}, token: @test_token)
  ...
  test "get_chat_member" do
    use_cassette "get_chat_member" do
      {:ok, chat_member} = Nadia.get_chat_member("@group", 666, %{}, token: @test_token)
  ...
  test "get_chat_administrators" do
    use_cassette "get_chat_administrators" do
      {:ok, [admin | [creator]]} = Nadia.get_chat_administrators("@group", %{}, token: @test_token)
  ...
  test "get_chat_members_count" do
    use_cassette "get_chat_members_count" do
      {:ok, count} = Nadia.get_chat_members_count("@group", %{}, token: @test_token)
      assert count == 2
    end
  end
  ...
  test "leave_chat" do
    use_cassette "leave_chat" do
      assert Nadia.leave_chat("@group", %{}, token: @test_token) == :ok
    end
  end
  ...
  test "answer_inline_query" do
    ...
    use_cassette "answer_inline_query" do
      assert :ok == Nadia.answer_inline_query(666, [photo], %{}, token: @test_token)
    end
  end

  test "get_sticker_set" do
    use_cassette "get_sticker_set" do
      assert {:ok, sticker_set} = Nadia.get_sticker_set("TomNJerry", %{}, token: @test_token)
  ...
  test "upload_sticker_file" do
    use_cassette "upload_sticker_file" do
      assert {:ok, file} =
        Nadia.upload_sticker_file(666, "BQADBAADzwADZgctUjisl0we_2qGAg", %{}, token: @test_token)
  ...
  test "set_sticker_position_in_set" do
    use_cassette "set_sticker_position_in_set" do
      assert :ok = Nadia.set_sticker_position_in_set("CAADBQADLgADmEjsA7jm5QOy8WxsAg", 0, %{}, token: @test_token)
    end
  end

  test "delete_sticker_from_set" do
    use_cassette "delete_sticker_from_set" do
      assert :ok = Nadia.delete_sticker_from_set("CAADBQADLgADmEjsA7jm5QOy8WxsAg", %{}, token: @test_token)
    end
  end
  ...
  test "send_animation" do
    use_cassette "send_animation" do
      ...
      {:ok, message} = Nadia.send_animation(666, file_id, %{}, token: @test_token)
  ...
  test "pin_chat_message" do
    use_cassette "pin_chat_message" do
      assert :ok = Nadia.pin_chat_message(666, 666, %{}, token: @test_token)
    end
  end

  test "unpin_chat_message" do
    use_cassette "unpin_chat_message" do
      assert :ok = Nadia.unpin_chat_message(666, %{}, token: @test_token)
    end
  end
end
